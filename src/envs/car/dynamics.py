...
class Car:
    ...
    def gas(self, gas):
        ...
        if self.allow_reverse:
            ...
            for w in self.wheels[2:4]:
                diff = gas - w.gas
                if gas > 0 and diff > +0.1:
                    diff = +0.1  # gradually increase, but stop immediately
                elif gas < 0 and diff < -0.1:
                    diff = -0.1  # no longer stops immediately
                w.gas += diff
    ...
    def step(self, dt):
        for w in self.wheels:
            ...
            w.omega += (
                dt * ENGINE_POWER * w.gas / WHEEL_MOMENT_OF_INERTIA / (abs(w.omega) + 5.0)
            ...
            w.ApplyForceToCenter(
                (p_force * side[0] + f_force * forw[0], p_force * side[1] + f_force * forw[1]),
                True,
            )
