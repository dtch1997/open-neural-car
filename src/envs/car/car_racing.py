...
class CarRacing(gym.Env, EzPickle):
    ...
    def check_timeout(self, reward, done):
        if self.t - self.last_touch_with_track > self.max_time_out and self.max_time_out > 0.0:
            ...
            for k, lanes in deepcopy(list(next_nodes.items())):
                for lane, direction in lanes.items():
                    ...
                    if FORBID_HARD_TURNS_IN_INTERSECTIONS:
                        ...
                        if (direction_tmp - (self.car.hull.angle + np.pi) + np.pi / 4) % (
                            np.pi * 2
                        ) < np.pi / 2:
                            remove = True
                    else:
    ...
    def _create_obstacles(self):
        ...
        for count, idx in enumerate(obstacle_tiles_ids):
            ...
            if self.random_obstacle_shape:
                ...
                p2 = (
                    x + 2 * width * math.cos(beta) + TRACK_DETAIL_STEP / 2 * math.sin(beta),
    ...
    def _create_info(self):
        ...
        for num_track in range(self.num_tracks):
            ...
            inters = np.logical_or(
                info[info["track"] == num_track]["t"], info[info["track"] == num_track]["x"]
            )
    ...
    def _set_lanes(self):
        if self.num_lanes_changes > 0 and self.num_lanes > 1:
            ...
            for pos, idx in enumerate(changes):
                ...
                if (
                    sum(((changes_in_track - idx) > 0) * ((changes_in_track - idx) < 10)) > 0
    ...
    def _generate_track(self):
        if self.load_tracks_from is not None:
            ...
            try:
                ...
                self.info[
                    ["count_left", "count_right", "count_right_delay", "count_left_delay"]
                ] = 0
    ...
    def _create_track(self):
        ...
        for j in range(len(self.track)):
            ...
            if SHOW_BETA_PI_ANGLE:
                ...
                self.track_lanes.append([[p1x, p1y], [p2x, p2y], [p3x, p3y], [p4x, p4y]])
                ...
                if self.info[j]["lanes"][lane]:
                    ...
                    if self.info[j]["end"] == False and self.info[j]["start"] == False:
                        ...
                        if (
                            ...
                            # If last tile didnt exist
                            if (
                                info_track[(j_relative + 1) % len(info_track)]["lanes"][lane]
                                == False
                            ):
                                last = True
                        ...
                        for i in [0, 1]:  # because there are two point to do
                            ...
                            if len(p3) == 0:
                                ...
                                while len(p3) == 0 and distance < PLAYFIELD:
                                    close = np.where(
                                        np.linalg.norm(np.subtract(p3_org, p1), axis=1) <= distance
                                    )[0]
        ...
        if self.num_tracks > 1:
            ...
            inter2 = np.array(
                [
                    ...
                    for x in points2
                    if (
                        np.linalg.norm(points1[:, 1, :] - x[1:], axis=1) <= TRACK_WIDTH / 3.5
                    ).sum()
                    >= 1
                ]
            )
            ...
            for i in range(inter2.shape[0]):
                if (
                    ...
                    or np.array_equal(inter2[i, 1, :], inter2[((i + 1) % len(inter2)), 0, :])
                    == False
                ):
            ...
            for i in range(intersections.shape[0]):
                ...
                if sec1 is not False and sec2 is not False:
                    ...
                    if remove:
                        ...
                        key = np.where(np.all(track1[:, :, [2, 3]] == sec1[0], axis=(1, 2)))[0]
                        val = np.where(np.all(track2[:, :, [2, 3]] == sec2[0], axis=(1, 2)))[
                            0
                        ] + len(track1)
                        intersection_keys.append(key[0])
                        intersection_vals.append(val[0])

                        key = np.where(np.all(track1[:, :, [2, 3]] == sec1[-1], axis=(1, 2)))[0]
                        val = np.where(np.all(track2[:, :, [2, 3]] == sec2[-1], axis=(1, 2)))[
                            0
                        ] + len(track1)
                        intersection_keys.append(key[0])
                        intersection_vals.append(val[0])
    ...
    def get_rnd_point_in_track(self, border=True):
        ...
        _, beta, x, y = self._get_rnd_position_inside_lane(idx, border=border, direction=direction)
        beta = 0  # hard-coded
        return [beta, x, y]
