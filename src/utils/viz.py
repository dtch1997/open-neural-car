...
def plot_trajectory(
    ...
    for i in range(len(state_trajectory)):
        ax[0, 0].scatter(
            state_trajectory[i, 0],
            state_trajectory[i, 1],
            s=5,
            c=colors[int(agent_history[i])],
    ...
    vel_goals = [
        np.ones(traj.shape[0]) * goal_states[i, 3]
        for i, traj in enumerate(partial_trajectory_list)
    ]
    ...
    acc_goals = [
        np.ones(traj.shape[0]) * goal_states[i, 5]
        for i, traj in enumerate(partial_trajectory_list)
    ]
    ...
    attitude_goals = [
        np.ones(traj.shape[0]) * goal_states[i, 2]
        for i, traj in enumerate(partial_trajectory_list)
    ]
    ...
    curvature_goals = [
        np.ones(traj.shape[0]) * goal_states[i, 4]
        for i, traj in enumerate(partial_trajectory_list)
    ]
